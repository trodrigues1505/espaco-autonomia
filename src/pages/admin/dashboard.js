...
export async function renderDashboard(container, page) {
  ...
  const [ocorrencias, alunos, feriados, notifs] = await Promise.all([
    ...
    (() => {
      ...
      return sb.from('feriados').select('*')
        .gte('data', hoje.toISOString().slice(0,10))
        .lte('data', ate.toISOString().slice(0,10)).order('data')
    })().then(r => r.data || []),
  ...
  container.innerHTML = `
    <div class="topbar">
      <div class="topbar-t">Dashboard</div>
      <div style="font-size:11px;color:var(--txt2)">${hoje.toLocaleDateString('pt-BR',{weekday:'long',day:'numeric',month:'long'})}</div>
    </div>
    <div class="content">
      ${renderPainelNotif(notifs, { titulo: 'Avisos do sistema' })}
      ...
      <div style="display:grid;grid-template-columns:3fr 2fr;gap:16px">
        <div style="background:#fff;border:1px solid var(--borda);border-radius:var(--r);overflow:hidden">
          <div style="padding:13px 18px;border-bottom:1px solid var(--borda)"><span style="font-family:'Cormorant Garamond',serif;font-size:16px;font-weight:500;color:var(--verde)">Aulas de hoje</span></div>
