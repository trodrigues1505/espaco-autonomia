...
export async function renderProfAulas(container, page) {
  ...
  const tipo = perfil?.tipo
    ...
    container.innerHTML = `
      <div class="topbar"><div class="topbar-t">Minhas Aulas</div><span style="font-size:11px;color:var(--txt2)">Próximos 14 dias</span></div>
      <div class="content">
        ${(ocs||[]).length===0?'<div style="text-align:center;padding:40px;font-size:13px;color:var(--txt2)">Nenhuma aula nos próximos 14 dias.</div>':
          (ocs||[]).map(oc=>{
            ...
            return `<div style="background:#fff;border:1px solid ${cancelada?'#f5c1c1':'var(--borda)'};border-radius:var(--r);padding:14px 16px;margin-bottom:10px;display:flex;align-items:center;justify-content:space-between;${cancelada?'opacity:.7':''}">
              ...
              <div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end">
                ${!cancelada?`
                  ...
                  <button onclick="window._currentPage='prof-aulas';cancelarOcorrenciaGrade('${oc.id}',${JSON.stringify({id:oc.id,data_hora:oc.data_hora,modalidade:oc.modalidade,confirmados:oc.confirmados,professor_id:oc.professor_id})})" style="padding:5px 12px;background:#fceaea;color:#8a1a1a;border:1px solid #f5c1c1;border-radius:6px;font-size:11px;cursor:pointer;font-family:'DM Sans',sans-serif">Cancelar aula</button>
